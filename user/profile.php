        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="card shadow-lg p-3 mx-2 mt-5">
                    <h4>Requiter Proflie</h4>
                    <?php
                    if(isset($_POST['update'])){
                        if(trim($_POST['Rname']) == "") {
                            $error_msg = "<p class='text-danger'>All fields requisred.</p>";
                        } elseif(strlen($_POST['Rname']) < 10 || strlen($_POST['Rname']) > 30){
                            $error_msg = "<p class='text-danger'>Name too small or large.</p>";
                        }
                        else{
                            $reqName = htmlentities(mysqli_real_escape_string($conn,$_POST['Rname']));
                            if(update_req_profile($conn,$reqName) == true) {
                    ...
                    <?php if(isset($error_msg)) { echo $error_msg; } ?>
                        <div class="form-group">
                            <input type="text" name="Rname" id="name" value="<?php echo   $_SESSION['req-name']; ?>"
                                class="form-control">
                        </div>
                        <div class="form-group">
